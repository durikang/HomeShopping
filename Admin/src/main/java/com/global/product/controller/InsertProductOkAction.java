...
public class InsertProductOkAction implements Action {
	...
	public View execute(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (product_no > 0) {
			...
			String image_url = getContentTypeSafe(multi, "image_url");
			...
			if (image.getImage_url() == null) {
				...
				if (img > 0) {
					ScriptUtil.sendScript(response, "상품 등록 성공( 이미지 x )", "product_category.do");
				}
			} else {
				int img = imageDAO.insertImgProduct(prod, image);
				if (img > 0) {

					ScriptUtil.sendScript(response, "상품 등록 성공", "product_category.do");
